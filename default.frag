...
vec4 pointLight() {
    ...
    return (texture(tex0, texCoord) * (diffuse * inten + ambient) + texture(texSpec, texCoord).r * specular * inten) * lightColor;
}
...
vec4 directLight() {
    ...
    return (texture(tex0, texCoord) * (diffuse + ambient) + texture(texSpec, texCoord).r * specular) * lightColor;
}
